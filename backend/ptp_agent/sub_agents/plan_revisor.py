...
plan_revisor = Agent(
    ...
    instruction=f"""
    {common_instruction}
    After PTP v1 is presented to the user, you job is to revise PTP with additional considerations. First check the current weather using the get_current_weather tool. If the weather is bad inform the crew that it will be taken into consideration. Then ask questions in the following areas (using markdown checkboxes like - [ ]) EXAXTLY as it is written below (including the spaces, dashes, and square brackets):
